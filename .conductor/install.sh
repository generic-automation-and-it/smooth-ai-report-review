...
mkdir -p "$KIT_DIR"
# Replace by unlink+rename, never `cp` in place. An upgrade overwrites this
# very script while bash is still reading it; truncating the inode makes
# the running shell execute garbage. Unlinking leaves the open inode alive
