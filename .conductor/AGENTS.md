...
Human-facing walkthrough and the manual UI paste-in alternative: see
`generic-automation-and-it/smooth-llm-imposter` → `.docs/wiki/setups/conductor.build-smooth-llm-imposter.md`.
Updated: 2026-07-30
...
- **The `claude-code` platform additionally keeps `--no-skills --no-hooks`.** Its skills/hooks resolve through
  the committed `.claude -> .agents` symlink into `.agents/skills/` (92 tracked files) and
