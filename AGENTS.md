...
| 2026-06-11 | npm opencode plugin `@generic-automation-and-it/smooth-ai-review` (root `package.json` + `opencode-plugin.js`) links skills into the consumer's `.agents/skills/` at startup; tag-triggered `npm-publish.yml` with version-lockstep guard (`NPM_TOKEN` Secret). | LADR-038 |
