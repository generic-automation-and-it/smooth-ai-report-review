...
| 2026-06-13 | `npm-publish.yml` publishes on every push to `main` (version-gated, no `paths` filter), manual dispatch, and semver tags; relaxes the lockstep guard for non-tag runs; skips already-published versions. The `paths` filter was dropped because it missed version-bump-only release commits on `main`; semver tag pushes remain covered because GitHub does not evaluate path filters for tag pushes. | LADR-038 |
