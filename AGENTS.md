...
- **Three distribution channels, one source of truth.** Consumers get the gate via the README copy-installer, via `workflow_call` (`uses: …/pipeline-code-review-report.yml@v1` — scripts fetched into `.smooth-ai-review-tools/` at run time), or get the skills via the Claude Code plugin `smooth-ai-review` (`.claude-plugin/` manifests; root `skills` is a symlink to `.agents/skills`; the plugin does NOT install the CI gate). The caller template `.docs/examples/code-review-caller.yml` duplicates the `model_preset` dropdown — extend it together with the workflow's preset mapping.
