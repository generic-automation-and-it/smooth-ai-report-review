...
The `gemini` provider in `.agents/skills/ai-review-report/assets/opencode.json` intentionally declares `npm: "@ai-sdk/google"` while being pointed at a gateway: when `OPENCODE_REVIEW_REPORT_GEMINI_URL` is set, `lib/setup-opencode-config.sh` injects it as `options.baseURL` in the **installed** config at runtime (LADR-034), so `@ai-sdk/google` talks to a LiteLLM proxy / OpenAI-compatible gateway surface (e.g. `…/v1beta/openai`) over HTTPS instead of Google's native endpoint. opencode is **provider-agnostic transport** — the SDK label does not dictate the host. The committed `opencode.json` deliberately ships **no `baseURL`** on this provider (it is injected at install time, never a static `{env:}` placeholder — an unset placeholder would substitute to an empty-string `baseURL`); an empty URL var simply falls back to the native base. This exact pairing is proven working in the prototype repo on every run. **DO NOT flag**: (a) the `@ai-sdk/google` + gateway-URL pairing as a "provider/SDK mismatch" or critical integration failure; (b) the absent `baseURL` in `assets/opencode.json` as a missing-config or broken-proxy bug (it is injected at runtime by design); nor **suggest** switching to `@ai-sdk/openai-compatible`. Any change to the provider transport is a deliberate engineering decision, not a review fix. (Recurring false positive: flagged Critical across multiple reviews, skipped each time.)
...
| 2026-06-09 | Reword DR-009 for LADR-034: the gemini provider's gateway `baseURL` is now injected at install time from `OPENCODE_REVIEW_REPORT_GEMINI_URL`. Added guard (b) — do not flag the deliberately-absent `baseURL` in the committed `opencode.json` as a missing-config/broken-proxy bug. |
