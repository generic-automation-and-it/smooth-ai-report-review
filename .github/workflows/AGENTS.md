...
- `run-review.sh`'s post-install block must explicitly export `PATH="$HOME/.opencode/bin:$PATH"` after the opencode installer runs. The installer writes to `$GITHUB_PATH` (a GitHub Actions special: it appends to PATH of SUBSEQUENT steps, not the current shell) and to `$HOME/.bashrc` (which `bash` does not re-source in the same process), so `command -v opencode` in the same step used to fail with `❌ opencode is not on PATH after install.` (this was the second PR #86 failure on CI run 30552219304). The post-install update also appends to `$GITHUB_PATH` for any later step that needs `opencode` on its PATH.
