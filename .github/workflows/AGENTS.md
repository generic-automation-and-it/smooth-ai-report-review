...
| Date | Change | Ref |
|------|--------|-----|
| 2026-07-30 | `pipeline-code-review-report.yml` job body refactored to a thin wrapper (Checkout PR head → Fetch review tooling → `run-review.sh`). The 27 inline `run:` blocks are now in `.agents/skills/ai-review-report/scripts/run-review.sh`, driven entirely by env vars and `$GITHUB_EVENT_PATH` (no `inputs.*` / `steps.*.outputs` / expression interpolation), so the same code drives the new local-job packaging (`.docs/examples/code-review-local.yml`) for consumers under `allowed_actions: "selected"`. The `Run review gate` step now forwards `GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}` in its `env:` block (regression fix from PR #86's CI run, where the preflight fired because bare `run:` shells do not auto-inject GITHUB_TOKEN). | — |
