...
`ai-review-report` is the **generator** half of the PR code-review skill pair (paired with `/ai-review`, the consumer). It runs as a GitHub Actions gate (`.github/workflows/pipeline-code-review-report.yml`), reviews PRs in chunks via the `opencode` CLI transport, and posts a structured review. SKILL.md is the runtime contract the review model follows; this file is the edit-time companion — LADR history, env-var provenance, confirmed-FP PR references, and the skill layout that the scripts reference by hardcoded path.

## Non-Negotiables

- **Workflow ↔ script paths are coupled.** The gate invokes skill scripts by hardcoded path (`.agents/skills/ai-review-report/scripts/…`). Moving or renaming a script, the skill folder, or the workflow file (`pipeline-code-review-report.yml`) silently breaks the gate. Change the workflow YAML and the scripts in the same commit.
...
- **Decision**: Publish the repo root as npm package `@generic-automation-and-it/smooth-ai-review` (`package.json` `files` whitelist: `opencode-plugin.js` + `.agents/skills/` minus `scripts/eval/`; the root `skills` symlink is excluded by npm pack's symlink handling and not whitelisted). `opencode-plugin.js` exports `SmoothAiReviewSkills`, which at startup links each of the three skills into the consuming worktree's `.agents/skills/<name>` — the *same* path the CI gate's locate step and every SKILL.md doc reference, so one path contract serves all channels. Links are created with `fs.symlinkSync(..., "junction")` (directory links without admin rights on Windows; plain dir symlinks on POSIX). Precedence: a real directory at the destination (copy-install/vendored) is never touched; an existing symlink is re-pointed only if stale (package cache moved on update). Link paths are appended idempotently to `.git/info/exclude` (local-only) so the consumer's tree and `.gitignore` stay clean. Everything is wrapped in try/catch — materialization failure warns but never breaks opencode startup. Publishing is tag-triggered (`.github/workflows/npm-publish.yml`, semver tags `v[0-9]+.[0-9]+.[0-9]+` only — the floating `v1` reusable-workflow tag can never re-publish) with a guard asserting `package.json` == `.claude-plugin/plugin.json` == tag version, `npm publish --access public --provenance`, `NPM_TOKEN` as a repo Secret (never a Variable).
- **Consequences**: opencode consumers add one `opencode.json` line and vendor nothing; skill-doc paths (`.agents/skills/...`) resolve verbatim. The eval harness (224KB of C# fixtures) is excluded from the package — eval work requires a clone. The npm package and the Claude Code plugin version in lockstep by CI guard. First-session caveat: links are created at session init, so skills may only be discovered from the next session — documented as "restart opencode once after first install". Verified on opencode 1.16.2: plugin loads at session init (not bare `opencode serve`), links + excludes created, vendored-dir precedence and stale-relink behavior confirmed from the packed tarball.
...
- **Two skills, opposite directions.** `ai-review-report` (this skill) *generates* the review (CI gate, or locally via `scripts/local-review.sh`). `ai-review` (invoked `/ai-review`) *consumes* a posted review and applies fix/skip decisions back to the PR. Don't conflate them or merge their scripts.- **opencode's `PermissionConfig` defines a fixed key set** (DR-011). The keys are `bash`, `edit`, `read`, `grep`, `glob`, `list`, `task`, `skill`, `external_directory`, `webfetch`, `websearch`, `lsp`, `todowrite`, `question`, `doom_loop` — and there is **no `write` key** (verified against `https://opencode.ai/config.json`). opencode silently ignores unknown permission keys, so a `permission.write: deny` suggestion is a no-op that only *looks* like a guard. Write protection is two-layer via `tools.write:false` + `permission.edit:deny`.
...
| 2026-06-09 | Added **LADR-034** (full: Date/Context/Decision/Consequences) — per-provider gateway `baseURL` injected at install time by `setup-opencode-config.sh` from `OPENCODE_REVIEW_REPORT_<P>_URL`, fixing Gemini-via-LiteLLM on installed deployments (failing run 27186819544). Added the env-provenance note that the per-provider URL is consumed as `baseURL` (not just presence-checked), and an editor Key Behavior (don't "fix" the deliberately-absent `baseURL`; wire new env-driven providers into `_inject_base_urls`; keep the `is_ours` `http(s)://` clause). SKILL.md carries only the slim rule-only LADR-034; DR-009 reworded. | — |
| 2026-06-09 | **Hardened the `review` agent + applied the valid bunker-PR-#5419 review findings.** LADR-029 + `opencode.json`: `bash` now disabled (`tools.bash:false` / `permission.bash:deny`) — closes the config↔doc drift where bash/web were *enabled* in the shipped config while the LADR text claimed all three disabled; `webfetch`/`websearch` kept enabled (intentional, now documented — the model verifies references). Also: dropped `OPENCODE_GATEWAY_API_KEY` from `$GITHUB_ENV` (`resolve-provider.sh` + workflow inline resolver — nothing downstream reads it), SHA-pinned `actions/cache@5a3ec84 # v4.2.3` (restore+save), and added `--paginate` + `-F`→`-f` fixes to `copilot-review.sh`. Not applied (false positives / copy-artifacts here): blanket `set -euo pipefail` (baseline `set -e`; `resolve-provider.sh` is `set`-less by design as it's sourced), static `baseURL` (we use LADR-034 runtime injection), stale `DR-010`/`-v2` refs, and `applyTo` frontmatter. | — |
| 2026-06-09 | **Trimmed SKILL.md to runtime-only; editor-context confirmed to live here.** Per the SKILL.md line-83 contract, removed editor content still in SKILL.md (all already in this file): the duplicate Skill-layout inventory (→ one-line pointer), the two confirmed-FP PR citations (#4992/#5258) in Non-Negotiables, and the history/provenance/supersede narratives inside LADR-002/009/011/021/022/023/025 + the supersede `Status` lines on LADR-023/026/027. **Corrected a stale claim** in **LADR-023** here: it stated the provider is declared with `baseURL={env:OPENCODE_REVIEW_REPORT_GEMINI_URL}` — reworded to past-tense history + a "current state (LADR-034)" note (committed config has no `baseURL`; injected at install per DR-009). No content needed adding to this file — it was already the canonical home. | — |
| 2026-06-10 | Added **LADR-035** + **LADR-036** (full Date/Context/Decision/Consequences) — hard chunk-prompt size enforcement (single-directory halving fallback, per-file diff truncation, 200KB chunk diff budget) and review-coverage-gaps-never-block + visible fail-closed override, fixing the bunker-procurement PR #5404 false-block class (run 27259917266; reviews 4465417128/4465489664). Added the PR #5404 row to the confirmed-FP list, the `test-chunk-prompt-budget.sh` Test References row + Skill Layout entry, and hardened the "Skill prompt is bounded" quality constraint (caps are enforced, not conventional). Drift fix: the `test-review-chunk-threshold.sh` Test References row claimed it verifies `OPENCODE_REVIEW_REPORT_MAX_FILE_COUNT` — it actually tests the `MIN_FILE_COUNT_BEFORE_CHUNCKING` single-chunk threshold. | — |
| 2026-06-10 | Added **LADR-037** (full Date/Context/Decision/Consequences) — the gate is callable as a reusable workflow: `on.workflow_call` inputs, all `github.event.inputs.*` → `inputs.*`, the nine script invocations routed through `$REVIEW_SKILL_DIR` (local skill tree wins; else `.smooth-ai-review-tools/` side checkout pinned to `tools_ref`/`job_workflow_sha`), installer perl guarded with `unless m{\.smooth-ai-review-tools}`. No script changes — all internal resolution is `$BASH_SOURCE`-based. Caller template at `.docs/examples/code-review-caller.yml`; Claude Code plugin manifests (`.claude-plugin/`, root `skills` symlink) added in the same change. Updated the path-coupling Key Behavior. | — |
