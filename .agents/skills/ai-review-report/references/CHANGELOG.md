...
| Date | Change | Ref |
|:-----|:-------|:----|
| 2026-07-04 | `pipeline-ai-analyse.yml` now prefers the optional `OPENCODE_ANALYSE_GH_TOKEN` Secret for the auto-fix push and falls back to `GITHUB_TOKEN` only when the PAT is unset. | LADR-042 |
...
| 2026-07-04 | Hardened privileged triggers: only `OWNER`/`MEMBER`/`COLLABORATOR` `/ai-review` comments can start a run; fork PR reviews force pinned `.smooth-ai-review-tools` instead of executing `.agents/skills/ai-review-report` from the PR head; and `pipeline-ai-analyse.yml` now always uses fetched tooling after PR checkout with persisted checkout credentials disabled, an explicit masked-token push, and both tooling-fetch steps defaulting to `v1` (floating major tag) instead of `main`. Caller template updated with the same trusted-comment guard and the same name-coupling required by `workflow_run`. | LADR-043 |
