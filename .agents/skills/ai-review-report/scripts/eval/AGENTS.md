...
- **A `must-not-flag` fixture must contain NOTHING legitimately flaggable at Critical/High/Medium — not just nothing matching its DR.** Precision scoring is deliberately severity-only (`run-evals.sh`, the `flagged_any` branch): any Critical/High/Medium on a must-not-flag fixture fails it. That rule is correct *because* the fixture is supposed to be a snapshot of code a reviewer should pass over in silence, and it is what makes the harness a real precision instrument — narrowing it to "did the reviewer re-raise this specific DR" would let a reviewer that flags something new and wrong on every fixture score 100%. The obligation therefore sits on the fixture: **an incidental defect in the `after/` tree is a fixture bug, and it will read as a precision regression forever.** Three of the fourteen carried one (run 30766652401): a stale `baseURL` in DR-009 that contradicted the DR-009 context injected alongside it, a genuine `${{ inputs.pr_number }}` script-injection vector in DR-015, and a `ToListAsync` whose result was only `.Count`ed in DR-007. The reviewer was right in all three cases; the corpus was wrong. When adding a fixture, read the `after/` tree as if you were the reviewer and remove anything you would legitimately raise.
