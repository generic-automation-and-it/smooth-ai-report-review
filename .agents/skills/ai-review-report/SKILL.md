...
- `assets/opencode.json` — provider config (`gemini`, `github-copilot`, `openai`) installed into opencode's global scope at job start
...
| `OPENCODE_API_HEALTH_OVERRIDE` | Forces the gateway health-probe path. Unset → derived from the URL's API surface: `/v1beta/models` (Google host — Gemini-native), else `/v1/models` (OpenAI-compatible, incl. a LiteLLM proxy fronting any provider), except Copilot → `/models`. Set to `/health` for a LiteLLM proxy's model summary. Always host-root-relative | GitHub **Variable** (optional) | exported in shell |
| `OPENCODE_PROVIDER_ID` | **Derived** by `lib/resolve-provider.sh` — the opencode.json provider key the model is prefixed with (`gemini` / `github-copilot` / `openai`); consumed by `lib/opencode-with-fallback.sh` | resolver step → `$GITHUB_ENV` | exported by `local-review.sh` |
...
**Local runs** use the same provider mechanism as CI. `local-review.sh` accepts `--provider`/`OPENCODE_PROVIDER`, harvests all three providers' `OPENCODE_<P>_URL` / `OPENCODE_<P>_API_KEY` pairs from the shell rc files (so the non-interactive skill subagent sees them), sources `lib/resolve-provider.sh` to pick + validate the selected pair, and runs a reachability preflight against the resolved gateway. GHA Variables are CI-only; locally the primary review model is the `--model` arg and the secondary/orchestrator fall back to script defaults (which must be overridden for non-GEMINI providers).

## TL;DR

GitHub Actions workflow that reviews PRs in chunks through the `opencode` CLI transport — provider-selectable via `OPENCODE_PROVIDER` (`GEMINI` / `COPILOT` / `OPENAI`) — loads project-specific `*AGENTS.md` context files, and posts structured reviews with priority categorization — incremental reviews MUST never approve PRs (only full reviews can).
