...
| `OPENCODE_REVIEW_REPORT_MODEL_PRIMARY_REVIEW` | Primary deep chunk-review model (LADR-002) | GitHub **Variable**, default `gemini-3.1-pro-preview`; `workflow_dispatch` input overrides | `--model` arg |
| `OPENCODE_REVIEW_REPORT_MODEL_SECONDARY_REVIEW` | Second (and last) review model — fallback if primary fails | GitHub **Variable**, default `gemini-2.5-pro` | script default |
...
### LADR-030: Max-file-count gate + `OPENCODE_REVIEW_*` → `OPENCODE_REVIEW_REPORT_*` env rename
