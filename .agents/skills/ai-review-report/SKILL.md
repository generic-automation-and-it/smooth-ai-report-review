...
- `assets/opencode.json` — provider config (`litellm-gemini`, `github-copilot`, `openai`) installed into opencode's global scope at job start
...
| `OPENCODE_GEMINI_URL` / `OPENCODE_GEMINI_API_KEY` | Gateway base URL + bearer token for the `litellm-gemini` provider | URL → GitHub **Variable**; key → **Secret** | exported in shell (gateway is VPN-gated) |
