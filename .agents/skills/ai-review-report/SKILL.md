...
- **Runner requirement**: Runs on `ubuntu-latest` (GitHub-hosted). This requires the LiteLLM proxy (`OPENCODE_GEMININ_URL`) to be reachable from GitHub-hosted runners — i.e. publicly routable over HTTPS, not private-network/VPN-only. If the proxy is ever moved back behind a private network, switch `runs-on` back to `self-hosted` (the job is network-bound — chunked to <100KB/call — so plain `self-hosted` suffices, not `self-hosted-high-memory`). `gh` CLI is pre-installed on `ubuntu-latest` and must NOT be installed/updated in the workflow.
