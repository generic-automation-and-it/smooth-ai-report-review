...
- `scripts/lib/` — shared helpers (`resolve-provider.sh`, `opencode-with-fallback.sh`, `setup-opencode-config.sh`)
...
| `OPENCODE_GO_OPENAI_URL` / `OPENCODE_GO_OPENAI_API_KEY` | Base URL + key for the `go-openai` provider — OpenCode's [OpenCode Zen](https://opencode.ai/docs/go/) gateway, OpenAI-compatible surface (`@ai-sdk/openai-compatible`, base `https://opencode.ai/zen/go/v1`). Serves `deepseek-v4-flash`, `deepseek-v4-pro`, `glm-5.1` | URL → **Variable**; key → **Secret** (only if used) | exported in shell |
| `OPENCODE_GO_ANTHROPIC_URL` / `OPENCODE_GO_ANTHROPIC_API_KEY` | Base URL + key for the `go-anthropic` provider — same Zen gateway, Anthropic-compatible surface (`@ai-sdk/anthropic`, base `https://opencode.ai/zen/go/v1`). Serves `minimax-m2.7`, `qwen3.7-plus`. Same Zen key as the OpenAI surface | URL → **Variable**; key → **Secret** (only if used) | exported in shell |
...
| `OPENCODE_API_HEALTH_OVERRIDE` | Forces the gateway health-probe path. Unset → derived from the URL's API surface: `/v1beta/models` (Google host — Gemini-native), else `/v1/models` (OpenAI-compatible, incl. a LiteLLM proxy fronting any provider), except Copilot → `/models` and the two OpenCode Go surfaces → `<baseURL>/models` (relative to the configured base, not host root). Set to `/health` for a LiteLLM proxy's model summary. Always host-root-relative (except the OpenCode Go default) | GitHub **Variable** (optional) | exported in shell |
...
| `OPENCODE_GATEWAY_URL` / `OPENCODE_GATEWAY_API_KEY` | **Derived** — the selected provider's creds copied to generic names for the reachability checks | resolver / Install Dependencies → `$GITHUB_ENV` | exported by `local-review.sh` |
| `OPENCODE_GATEWAY_HEALTH_URL` | **Derived** — gateway host root + resolved health path (per-provider default or `OPENCODE_API_HEALTH_OVERRIDE`); what the health probe hits | resolver / Install Dependencies → `$GITHUB_ENV` | exported by `local-review.sh` |
...
**Local runs** use the same provider mechanism as CI. `local-review.sh` accepts `--provider`/`OPENCODE_PROVIDER`, harvests all five providers' `OPENCODE_<P>_URL` / `OPENCODE_<P>_API_KEY` pairs from the shell rc files (so the non-interactive skill subagent sees them), sources `lib/resolve-provider.sh` to pick + validate the selected pair, and runs a reachability preflight against the resolved gateway. GHA Variables are CI-only; locally the primary review model is the `--model` arg and the secondary/orchestrator fall back to script defaults (which must be overridden for non-GEMINI providers).
...
- **Date**: 2026-05-29
- **Status**: Partially superseded by LADR-026 — the *single-provider* decision is replaced by env-based provider selection (`OPENCODE_PROVIDER`). The local reachability preflight + `timeout`-shim parts remain in force.
...
- **Why partially superseded**: the "one provider everywhere" stance was reversed by LADR-026 — the provider is now env-selectable (GEMINI / COPILOT / OPENAI) and the endpoint can be a LiteLLM proxy *or* a provider's native API. The reachability preflight now probes the resolved endpoint with the right auth header (see LADR-026).
...
- **Decision**: Split OpenCode Go into **two** providers rather than one, each with its own `npm` + `baseURL` + credential namespace:
  - **`go-openai`** — `npm: "@ai-sdk/openai-compatible"`, `baseURL={env:OPENCODE_GO_OPENAI_URL}`, `apiKey={env:OPENCODE_GO_OPENAI_API_KEY}`. Models: `deepseek-v4-flash`, `deepseek-v4-pro`, `glm-5.1`.
  - **`go-anthropic`** — `npm: "@ai-sdk/anthropic"`, `baseURL={env:OPENCODE_GO_ANTHROPIC_URL}`, `apiKey={env:OPENCODE_GO_ANTHROPIC_API_KEY}`. Models: `minimax-m2.7`, `qwen3.7-plus`.

  Both `baseURL`s are the shared base `https://opencode.ai/zen/go/v1` — the respective SDK appends `/chat/completions` vs `/messages`. The same OpenCode Zen API key works for both surfaces (set whichever Secret you use). Two selectors map in: `OPENCODE_PROVIDER=OPENCODE-GO-OPENAI` → provider-id `go-openai`, `OPENCODE-GO-ANTHROPIC` → `go-anthropic` (resolved in `lib/resolve-provider.sh`, the workflow `OPENCODE_PROVIDER_ID` map + bootstrap creds case, `local-review.sh` cred harvest). `setup-opencode-config.sh`'s `is_ours` predicate widened to `["gemini","github-copilot","go-anthropic","go-openai","openai"]` (jq-sorted). The model-family fail-fast still applies (non-GEMINI must not carry a `gemini*` id). Health probe for both surfaces lists at **`<baseURL>/models`** (base has a `…/zen/go/v1` path prefix, so not the host-root `/v1/models` the other OpenAI-compatible providers use); `OPENCODE_API_HEALTH_OVERRIDE` still wins. Unlike the other providers (which omit `baseURL` and use the SDK native base), both go providers **declare** `baseURL` — the Zen endpoint has no SDK-native default — so `OPENCODE_GO_*_URL` feeds both opencode and the health probe.
- **Consequences**: Two switchable providers covering both OpenCode Go surfaces; no workflow edit to enable (URL = Variable, key = Secret, models = Variables). A run picks ONE surface — its model chain must be all-OpenAI-surface or all-Anthropic-surface ids (mixing won't resolve). Explicit `npm`/`baseURL` means we don't depend on opencode's catalog knowing `opencode-go`. Reaching usage beyond plan limits is handled OpenCode-side via the "Use balance" console toggle (Zen balance fallback) — not a pipeline concern.
