...
|----------|---------|-----------|--------------|
| `OPENCODE_LITELLM_URL` | LiteLLM gateway base URL (Gemini-native endpoint) for the `litellm-gemini` provider | GitHub **Secret** | exported in shell (gateway is VPN-gated) |
| `OPENCODE_LITELLM_API_KEY` | Bearer token for the LiteLLM gateway | GitHub **Secret** | exported in shell |
