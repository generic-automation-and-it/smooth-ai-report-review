...
- **Consequences**: Local runs need network access to the gateway (the CI runner reaches it over HTTPS; see the runner note in `AGENTS.md`). Off-VPN fails in ~8s with clear guidance instead of hanging. No personal Google key is involved anywhere. If a Google-key path is ever wanted for the team, re-introduce a provider switch.
