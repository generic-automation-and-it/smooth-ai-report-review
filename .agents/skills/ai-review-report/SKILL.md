...
Automated PR code review using chunked processing, context-aware analysis, and provider-selectable models (Copilot (default) / Gemini / OpenAI / OpenCode-Go, chosen by `OPENCODE_PROVIDER`) served through the `opencode` CLI. Updated: 2026-06-07. Maintainer: Platform Engineering.
...
|----------|---------|-----------|--------------|
| `OPENCODE_PROVIDER` | Selects the active provider: `COPILOT` (default), `GEMINI`, `OPENAI`, `OPENCODE-GO-OPENAI`, or `OPENCODE-GO-ANTHROPIC`. `lib/resolve-provider.sh` maps it to the provider-id + gateway creds | GitHub **Variable**, default `COPILOT` | `--provider` arg / shell, default `COPILOT` |
| `GH_TOKEN` | GitHub token with Copilot access — the only credential for the `github-copilot` provider (default). **No URL var** — the `@ai-sdk/github-copilot` SDK endpoint `https://api.githubcopilot.com` is fixed (LADR-032). NOTE: `gh` CLI prefers `GH_TOKEN` over `GITHUB_TOKEN`, so the CI Secret needs repo + pull-requests scope | key → **Secret** | `gh` CLI / shell export |
| `OPENCODE_GEMINI_URL` / `OPENCODE_GEMINI_API_KEY` | Gateway base URL + bearer token for the `gemini` provider | URL → GitHub **Variable**; key → **Secret** (only if used) | exported in shell |
...
| `OPENCODE_MODEL_PRIMARY_REVIEW` | Primary deep chunk-review model (LADR-002) | GitHub **Variable**, default `gpt-5.4`; `workflow_dispatch` input overrides | `--model` arg |
| `OPENCODE_MODEL_SECONDARY_REVIEW` | Second (and last) review model — fallback if primary fails | GitHub **Variable**, default `gpt-5.5` | script default |
| `OPENCODE_MODEL_ORCHESTRATOR` | Cheap model for semantic grouping + aggregation + summary (LADR-022); falls back to the resolved review model | GitHub **Variable**, default `gpt-5.4-mini` | script default |
...
**Secrets vs Variables**: **API keys / tokens** are credentials → **Secrets** (never Variables — those are plaintext and printable in logs); this includes Copilot's `GH_TOKEN`. Gateway **URLs**, the provider selector, and model ids are non-sensitive config → **Variables**, so they can be retuned without editing the workflow. Each model `vars.*` has a literal default so an unset Variable never blanks the model.

**Default provider COPILOT is key-only** (LADR-032): the `github-copilot` provider authenticates with `GH_TOKEN` (a GitHub token with Copilot access) and has **no URL var** — the `@ai-sdk/github-copilot` SDK endpoint is fixed, like the two OpenCode Go surfaces. The default model chain is the GPT chain (`gpt-5.4` primary / `gpt-5.5` secondary / `gpt-5.4-mini` orchestrator).

**Provider fail-fast**: for any non-default provider you MUST set the three `OPENCODE_MODEL_*` to that provider's models — `gemini-3.1-pro-preview` / `gemini-2.5-pro` / `gemini-3-flash-preview` (`GEMINI`), `gpt-5.4` / `gpt-5.5` / `gpt-5.4-mini` (Copilot/OpenAI), `deepseek-v4-pro` / `deepseek-v4-flash` / `glm-5.1` (`OPENCODE-GO-OPENAI`), or `qwen3.7-plus` / `minimax-m2.7` (`OPENCODE-GO-ANTHROPIC`); `lib/resolve-provider.sh` aborts the run if a `gemini*` model is used on a non-`GEMINI` provider (or a non-`gemini*` model on `GEMINI`), or the selected provider's creds are missing. (Also do not mix surfaces: an OpenAI-surface model id won't resolve on `go-anthropic` and vice-versa.)

**Local runs** use the same provider mechanism as CI. `local-review.sh` accepts `--provider`/`OPENCODE_PROVIDER`, harvests every provider's credentials from the shell rc files (`GH_TOKEN` for Copilot; URL + key for Gemini/OpenAI; API key only for the two OpenCode Go surfaces, whose base URL is hardcoded) (so the non-interactive skill subagent sees them), sources `lib/resolve-provider.sh` to pick + validate the selected credentials, and runs the provider-agnostic opencode health check (`lib/opencode-health.sh`: `opencode serve` + `/global/health`). GHA Variables are CI-only; locally the primary review model is the `--model` arg (default `gpt-5.4`) and the secondary/orchestrator fall back to script defaults (`gpt-5.5` / `gpt-5.4-mini`; must be overridden for non-Copilot providers).
...
### LADR-032: COPILOT is the default provider, key-only via `GH_TOKEN`

- **Date**: 2026-06-07
- **Status**: Accepted (changes the LADR-026 default from GEMINI to COPILOT; makes the `github-copilot` provider key-only like the LADR-027 Go providers)
- **Context**: We want the gate to run on GitHub Copilot by default, authenticated with the user's own `GH_TOKEN` (the GitHub token with Copilot access that `gh` CLI already exports locally) rather than a separate gateway URL + API-key pair. The `@ai-sdk/github-copilot` SDK has a fixed built-in endpoint (`https://api.githubcopilot.com`), so — exactly like the OpenCode Go providers (LADR-027) — there is no per-deployment URL to retune; the only credential is the token.
- **Decision**: (a) `opencode.json` `github-copilot.options.apiKey` → `{env:GH_TOKEN}`. (b) Default `OPENCODE_PROVIDER` flipped GEMINI → **COPILOT** in `lib/resolve-provider.sh`, `local-review.sh`, and the workflow `env:` (+ the `OPENCODE_PROVIDER_ID` fallback expression and the Install-Dependencies bootstrap case default). (c) COPILOT becomes **key-only**: in the resolver its `_rp_url_var=""` with `_rp_url_fixed="https://api.githubcopilot.com"` and `_rp_key_var="GH_TOKEN"` (mirrors the Go providers — fixed URL satisfies the presence check, only the key is env-driven); the workflow bootstrap case sets `GW_URL="https://api.githubcopilot.com"; K=GH_TOKEN`. (d) Default model chain → the GPT chain: `gpt-5.4` primary / `gpt-5.5` secondary / `gpt-5.4-mini` orchestrator (in the workflow `vars.*` fallbacks and the `local-review.sh` defaults). (e) `setup-opencode-config.sh`'s `is_ours` self-heal predicate relaxed: the apiKey discriminator now accepts `^{env:(OPENCODE_|GH_TOKEN)` so a config carrying `{env:GH_TOKEN}` for github-copilot still reads as managed. The OLD `OPENCODE_COPILOT_URL` Variable / `OPENCODE_COPILOT_API_KEY` Secret are retired.
- **Consequences**: A bare run (no Variables set) reviews on Copilot with the GPT chain and a single credential — the developer's `GH_TOKEN` locally, a `GH_TOKEN` Secret in CI. Switching to GEMINI/OPENAI/OpenCode-Go still works via `OPENCODE_PROVIDER` + that provider's key (+ URL where relayed) + `OPENCODE_MODEL_*`; the model-family fail-fast is unchanged. Trade-off: `gh` CLI prefers `GH_TOKEN` over `GITHUB_TOKEN`, so the job-scoped `GH_TOKEN` becomes the token `gh` uses in steps that don't set `GITHUB_TOKEN` explicitly — the CI `GH_TOKEN` Secret therefore needs repo + pull-requests scope (documented in the workflow + the env table). Whether a raw GitHub PAT is accepted by `@ai-sdk/github-copilot` (vs a Copilot-exchanged token) is what this wiring is meant to exercise.
