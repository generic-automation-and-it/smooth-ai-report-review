...
|---|------|----------------------------|----------|------------------------|-----------------------|
   ...
   **Never rewrite an identifier as `#1` (LADR-067).** GitHub autolinks `#` followed by digits to an issue or PR, so `#1` anywhere you post renders as a link carrying that issue's title and leaves a cross-reference on it. Reproduce the identifier exactly as the review printed it — `4.` for a finding, `R2)` for a residual risk.
